...
class IPAdapterLitModule(LightningModule):
    ...
    def model_step(self, batch):
        ...
        with torch.no_grad():
            text_features = self.text_encoder(batch["text_input_ids"])[
                0
            ]  # encoder_hidden_states shape : bs 77 1024

        with torch.no_grad():
            image_embeds = self.image_encoder(batch["clip_images"]).image_embeds  # [bs,512]
