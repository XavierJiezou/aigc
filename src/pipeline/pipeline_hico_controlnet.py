...
class StableDiffusionHicoControlNetPipeline(
    DiffusionPipeline,
    ...
    def get_res_sample(self, latents: torch.Tensor, timesteps: torch.Tensor, cls_text,cls_mask,fuse_type="avg"):
        bs = latents.shape[0]
        with torch.no_grad():
            cls_text = self.reshape_tensor(cls_text)  # [bs*19,77]
            controlnet_encoder_hidden_states = self.text_encoder(cls_text)[0] # [bs*19,77,768]

        expanded_latents = latents.unsqueeze(1).repeat(
            1, 19, 1, 1, 1
        )  # [2, 19, 4, 64, 64]
        expanded_timesteps = timesteps.unsqueeze(0).unsqueeze(0).repeat(bs, 19)  # [2, 19]
        controlnet_image = cls_mask
        ...
        for res_sample in down_block_res_samples:
            ...
            if fuse_type == "avg":
                ...
                raise ValueError(f"Unsupported fuse_type '{fuse_type}', expected 'avg' or 'sum'.")
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # 4.2 prepare cls text:
        cls_text = cls_text.to(device)
        if self.do_classifier_free_guidance and not guess_mode:
            cls_text = torch.cat([cls_text] * 2)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                down_block_res_samples, mid_block_res_sample = self.get_res_sample(
                    latents=control_model_input,timesteps=t,cls_text=cls_text,cls_mask=image,fuse_type=fuse_type
                )
